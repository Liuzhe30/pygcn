...
class Processor:
    ...
    def data_pre_processing(self, fasta_path, rasa_path, window_length, tag):
        ...
        while line:
            ...
            #print(code[t:-t][:, :-1].shape)
            #print(np.array(ss_train).shape)
            #print(np.array(rasa_train).shape)
            code = np.delete(code, 20, axis=1)
            
            big_map = np.c_[(np.c_[code[t:-t],np.transpose(np.array(ss_train))]), np.transpose(np.array(rasa_train))]
            print(big_map.shape)
            
            x_train = np.r_[x_train, big_map]
        ...
        np.savetxt("/home/liuz/8_pytorch_gcn/data/rasa.content", x_train, fmt=' '.join(['%d']*21 + ['%.2f']*30 + ['%d']*2))     
...
if __name__=='__main__':
    ...
    tag = "test"
    fasta_path = "/home/liuz/8_pytorch_gcn/data/join.fasta"
